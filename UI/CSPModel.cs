...
namespace UI
{
    class CSPModel
    {
        ...
        const string LIB_NAME = "Lib_HomeScan.dll";// "Lib_v0+equal+know.dll";
        ...
        public bool ToFullModel(string mainProcessName)
        {
            ...
            {
                Console.WriteLine("TEST CPSModel.cs... !!!!!! " + channel);
				string addNewMessage = "";
				string messages = "";


				int maxNo = 3;
				for (int j = 2; j <= maxNo; j++) {
					for (int i = 1; i < j; i++)
					{
						if (i == 1)
						{
						...
						addNewMessage = addNewMessage + "addKnwlge{" + ATTACKER_KNOWLEDGE_VARIABLE_NAME + ".addKnowledge(msg" + 1 + ")}->";
					}
					channelAttTexts.Add(channel + "?"+ messages+ addNewMessage + ATTACKER_PROCESS);
				}

				//channelAttTexts.Add(channel + "?msg1{" + ATTACKER_KNOWLEDGE_VARIABLE_NAME + ".addKnowledge(msg1)}->" + ATTACKER_PROCESS);
				//channelAttTexts.Add(channel + "?msg1.msg2->addKnowledge{" + ATTACKER_KNOWLEDGE_VARIABLE_NAME + ".addKnowledge(msg1)}->addKnowledge{" + ATTACKER_KNOWLEDGE_VARIABLE_NAME + ".addKnowledge(msg2)}->" + ATTACKER_PROCESS);
				//channelAttTexts.Add(channel + "?msg1.msg2.msg3->addKnowledge{" + ATTACKER_KNOWLEDGE_VARIABLE_NAME + ".addKnowledge(msg1)}->addKnowledge{" + ATTACKER_KNOWLEDGE_VARIABLE_NAME + ".addKnowledge(msg2)}->addKnowledge{" + ATTACKER_KNOWLEDGE_VARIABLE_NAME + ".addKnowledge(msg3)}->" + ATTACKER_PROCESS);

				foreach (var ints in intMessages)
                {
                    //resendAttTexts.Add("           []" + channel + "!" + ints + "->" + ATTACKER_PROCESS);
                    Console.WriteLine("TEST CPSModel.cs...ToFullModel() !!!!!! " + ints);
                    resendAttTexts.Add("           []if (" + ATTACKER_KNOWLEDGE_VARIABLE_NAME + ".knows(" + ints + ")==true){" + channel + "!" + ints + "->" + ATTACKER_PROCESS + "}else{" + ATTACKER_PROCESS + "}");

                }
                foreach (var msg in constantMessages){
                    Console.WriteLine("TEST CPSModel.cs...ToFullModel() !!!!!! " + msg);
